document.addEventListener("DOMContentLoaded", function () {
  ...
  (async () => {
    try {
      ...
      document.querySelectorAll(".btn-mat-pdf").forEach((el) => {
        el.addEventListener("click", function () {
          ...
          const consultaAlumno = async () => {
            try {
              let data = { cmd: "informe-consulta", id: aluId, per: periodo, user_id: useId };
              ...
              if (dataServer.respuesta === "exito") {
                ...
                const promesas = ordenMaterias.map(async (materia) => {
                  ...
                  if (dataServer_ftz.respuesta === "exito") {
                    ...
                    let ftz_array = datNot['m-' + materia] && datNot['m-' + materia]["ftz"] ? datNot['m-' + materia]["ftz"].split("+") : [];
                    ftz_final = ftz_array.length > 0 ? dat_ftz[ftz_array[0]][ftz_array[1]][ftz_array[2]][ftz_array[3]] : 'S/N';

                    let deb_array = datNot['m-' + materia] && datNot['m-' + materia]["deb"] ? datNot['m-' + materia]["deb"].split("+") : [];
                    deb_final = deb_array.length > 0 ? dat_ftz[deb_array[0]][deb_array[1]][deb_array[2]][deb_array[3]] : 'S/N';

                    let rec_array = datNot['m-' + materia] && datNot['m-' + materia]["rec"] ? datNot['m-' + materia]["rec"].split("+") : [];
                    rec_final = rec_array.length > 0 ? dat_ftz[rec_array[0]][rec_array[1]][rec_array[2]][rec_array[3]] : 'S/N';

                    let ncnFinal = datNot['m-' + materia] && datNot['m-' + materia]["ncn"] ? datNot['m-' + materia]["ncn"] : 0;
                ...
                if (alumSector != 'TRA') {
                  const notIcfes = (async () => {
                    ...
                    icf_rangos = '0 - 221 (INSUFICIENTE), 222 - 325 (BÁSICO), 326 - 437 (SATISFÁCTORIO), 438 - 500 (AVANZADO)'
